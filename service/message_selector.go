...
func NewMessageSelector(repo repo.Repo,
	...
	logger.Infof("defalut timeout: %v, sign message timeout: %v, estimate message timeout: %v", cfg.DefaultTimeout,
		cfg.SignMessageTimeout, cfg.EstimateMessageTimeout)
...
func (messageSelector *MessageSelector) selectAddrMessage(ctx context.Context, appliedNonce *utils.NonceMap, addr *types.Address, ts *venusTypes.TipSet, maxAllowPendingMessage uint64) (*MsgSelectResult, error) {
	...
	for index, msg := range messages {
		...
		sigI, err := handleTimeout(messageSelector.walletClient.WalletSign, signMsgCtx, []interface{}{msg.WalletName, addr.Addr, unsignedCid.Bytes(), venusTypes.MsgMeta{
