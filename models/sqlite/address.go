...
func FromAddress(addr *types.Address) *sqliteAddress {
	sqliteAddr := &sqliteAddress{
		...
		GasPremiumRation:  addr.GasPremiumRation,
...
func (s sqliteAddress) Address() (*types.Address, error) {
	...
	return &types.Address{
		...
		GasPremiumRation:  s.GasPremiumRation,
...
func (s sqliteAddressRepo) UpdateFeeParams(ctx context.Context, addr address.Address, gasOverEstimation, gasPremiumRation float64, maxFee, maxFeeCap big.Int) error {
	...
	if gasPremiumRation != 0 {
		updateColumns["gas_premium_ration"] = gasPremiumRation
	}
