...
func TestAddress(t *testing.T) {
	...
	addressRepoTest := func(t *testing.T, addressRepo repo.AddressRepo) {
		...
		checkField := func(t *testing.T, expect, actual *types.Address) {
			...
			assert.Equal(t, expect.GasPremiumRation, actual.GasPremiumRation)
		...
		t.Run("UpdateFeeParams", func(t *testing.T) {
			...
			gasPremiumRation := 1.2
			assert.NoError(t, addressRepo.UpdateFeeParams(ctx, addr, gasOverEstimation, gasPremiumRation, maxFee, maxFeeCap))
			...
			assert.Equal(t, gasPremiumRation, r.GasPremiumRation)
		})
